...
* Sed [0%]
+ [ ] Implement a ~sed~ command to change a tag being sure that it will not attempt to change the anything that is not a tag (the rationale being that sometimes a careless use so the standard ~sed~ command may result in not only tags being changed, but also other words in the body of the text).
* Config [50%]
...
+ [ ] Add possibility to refile headlines that match more than one tag
* Runtime
+ [ ] Add possibility to declare the ~org-directory~ at runtime (for every sub command) via a ~-d~ flag.
+ [X] Add ~--exclude=<pattern>~ and ~--include=<pattern>~ to override at runtime the files to be searched.
